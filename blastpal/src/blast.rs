...
pub fn run(search: &HmmSearch) -> Vec<Blast> {
    ...
    let outfile = format!("{}/blast/{}-{}-{}-{}.blast", 
    ...
    if Path::new(&outfile).exists() {
        fs::remove_file(&outfile)
            .expect("Unable to delete previous blast results file");
    }

    // Get blastdb
    let blastdb = if CONFIG.report.blastdb == "" {
        format!("{}/{}/blast/{}", CONFIG.report.sets_dir, CONFIG.report.set_name, CONFIG.report.set_name)
    } else {
        format!("{}", CONFIG.report.blastdb)
    };

    // Run blastp command
    let output = Command::new(&CONFIG.programs.blast).args([
        "-outfmt",
    ...
    // Log
    info!("Completed blast of hmm search id# {} with {} at file: {}", search.id, output.status, outfile);
    ...
    info!("Found {} blast results for hmm search id# {}, gene {}", blasts.len().to_string(), search.id, search.gene_id);

    // Delete tmpfile
    fs::remove_file(&tmpfile)
        .expect("Unable to delete temporary query file");
...
fn generate_tmpfile(search: &HmmSearch) -> String {
    ...
    // Get filename
    let tmpfile = format!("{}/tmp/{}.hmm-{}", CONFIG.report.output_dir, search.gene_id, search.id);

    // Save to tmpfile
    let path = Path::new(&tmpfile);
    let mut fh = match File::create(&path) {
        Ok(res) => res,
        Err(e) => panic!("Unable to open temporary file for writing, {}, error: {}", tmpfile, e)
    };
...
fn gather_blast_results(outfile: &String, search: &HmmSearch) -> Vec<Blast> {
    ...
    let fh = match File::open(&outfile) {
        Ok(res) => res,
        Err(e) => panic!("Unable to open blast result file at {}, error: {}", outfile, e)
    };
    ...
    for ln in lines {
        ...
        let log_evalue = if tmp_evalue == 0.00 { -999.00 } else { tmp_evalue.log2().floor() };
