...
impl Sqlite {
    ...
    pub fn new() -> Self {
        ...
        if !Path::new(&BIODB_ARGS.sqlite_file).exists() {
            panic!("SQLite database file does not exist at {}.", BIODB_ARGS.sqlite_file);
        }

        // Connect to database
        let conn = Connection::open(&BIODB_ARGS.sqlite_file).unwrap_or_else(|_error| {
            panic!("Unable to open SQLite database at {}", &BIODB_ARGS.sqlite_file)
        });
    ...
    pub fn get_total(&self, table_name: &str) -> u64 {
        ...
        let total: u64 = self.conn.query_row::<u64, _, _>(&sql, [], |row| row.get(0)).unwrap();
        total
    }
    ...
    pub fn get_blast_results(&self, hmmsearch_id: &u32) -> Result<Vec<Blast>, Error> {
        ...
        let mut rows = match stmt.query([&hmmsearch_id]) {
            Ok(r) => r,
            Err(e) => panic!("Unable to execute SQL to obtain blast results, error: {}", e)
        };
