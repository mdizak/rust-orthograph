...
pub fn generate(hit: &Hit, is_extended: bool) -> Option<OrfTranscript> {
    ...
    if !CONFIG.switch.frameshift_correction {
        return Some( OrfTranscript {
            ...
            cdna_end_transcript: ((hit.ali_start - 1) * 3) + ((hit.ali_end - hit.ali_start + 1) * 3),
    ...
    let output = Command::new(&CONFIG.programs.exonerate).args([
        "--bestn",
        "1",
        "--score",
    ...
    if !output.status.success() {
        warn!("Did not receive successful exit code from exonerate, skipping transcript.  Error: {}", String::from_utf8_lossy(&output.stderr));
        return None;
    }

    // Log
    info!("Completed exonerate search of hmm search id# {} gene {} with {}, (stderr: {})", 
        hit.hmmsearch_id, hit.gene_id, output.status, String::from_utf8_lossy(&output.stderr));
    ...
    Some( OrfTranscript {
        ...
        cdna_start_transcript: cdna.coord_start + 1 + (hit.ali_start * 3) - 3,
        cdna_end_transcript: cdna.coord_end + (hit.ali_start * 3) - 3,
        aa_start_transcript: ((cdna.coord_start + ((hit.ali_start * 3) - 3) / 3) as f32).floor() as u16,
        aa_end_transcript: (((cdna.coord_end + (((hit.ali_start + 1) * 3) - 3)) / 3) as f32).ceil() as u16,
        aa_start_hmm: hit.hmm_start + (cdna.coord_start + ((hit.ali_start * 3) - 3) / 3),
        aa_end_hmm: hit.hmm_start + ((cdna.coord_end + ((hit.ali_start * 3) - 3)) / 3)
    })
...
fn read_exonerate_response<'a>(fasta: &'a HashMap<String, FastaResult>) -> Option<(&'a FastaResult, &'a FastaResult)> {
...
fn translate(cdna_seq: &String) -> Option<String> {
    ...
    let output = Command::new(&CONFIG.programs.translate).args([
        "--geneticcode",
        "1",
        "-F",
        "1",
