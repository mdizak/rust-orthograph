...
use crate::algorithms::{extract_reciprocal_hits, env_pseudo_master, frameshift_correction, hmm_overlap};
...
impl Reporter {
    ...
    fn initialize(&self) -> ReporterKit {
        ...
        let set_id: u32 = match db.get_set_id() {
            Ok(res) => res,
            Err(e) => panic!("Unable to determine id# for set {}, error: {}", &CONFIG.report.set_name, e)
        };
        info!("Got set id# {} for set name {}", set_id, CONFIG.report.set_name);

        // Get referenced taxa
        let ref_taxa: Vec<String> = if CONFIG.report.reference_taxa.len() > 0 {
            CONFIG.report.reference_taxa.split(",").map(str::to_string).filter(|i|i != "").collect::<Vec<String>>()
        } else {
            db.get_reference_taxa(&set_id).unwrap()
        };
        info!("Obtained {} reference taxa to use.", ref_taxa.len().to_string());
        ...
        info!("Obtained total of {} genes with aa sequences for reporting.", aaseq.len().to_string());
