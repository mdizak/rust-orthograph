...
impl Stats {
    ...
    pub fn new() -> Self {
        ...
        // Open files
        let brh_fh = biotools::io::open_file(format!("{}/best-reciprocal-hits.txt", CONFIG.report.output_dir));
        let nolap_fh = biotools::io::open_file(format!("{}/non-overlapping-best-reciprocal-hits.txt", CONFIG.report.output_dir));
        ...
        let filter_fh = biotools::io::open_file(format!("{}/filtered-hits.txt", CONFIG.report.output_dir));
    ...
    pub fn write_brh(&mut self, hit: &Hit) {
        let line = format!("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n",
            hit.gene_id, hit.header_full, hit.ali_start, hit.ali_end, hit.score, hit.evalue, hit.hmm_start, hit.hmm_end);

        self.brh_fh.write_all(&line.as_bytes())
    ...
    pub fn write_nolap(&mut self, hit: &Hit) {
        ...
        self.nolap_fh.write_all(&line.as_bytes())
            .expect("Unable to write to non-overlapping-best-hits.txt file");

    }

    pub fn write_filtered_hit(&mut self, gene_id: &String, header: &String, revcomp: &u8, translate: &u8, reason: &str) {

        // Format line
        let tmp_header = biotools::format_header(&header, &revcomp, &translate);
        let line = format!("{},{},{}\n", gene_id, tmp_header, reason);

        self.filter_fh.write_all(&line.as_bytes())
    ...
    pub fn delete_hit(&mut self, kit: &ReporterKit, hit_id: &u32) {
        ...
        match kit.memdb.execute(&sql, []) {
            Ok(_r) => { },
            Err(e) => panic!("Unable to delete hit from temporary hits table, id# {}, error: {}", hit_id, e)
        };
    ...
    pub fn discard_hmm_overlap(&mut self, kit: &ReporterKit, hit: &HmmDiscard) {
        ...
        self.write_filtered_hit(&hit.gene_id, &hit.header_base, &hit.revcomp, &hit.translate, "hmm-overlap");
    ...
    pub fn discard_env_pseudo_master(&mut self, kit: &ReporterKit, cand: &EnvCandidate) {
        ...
        self.write_filtered_hit(&cand.gene_id, &cand.header_base, &cand.hdr_revcomp, &cand.hdr_translate, "env-pseudo-master");
        self.discard_env_pseudo_master += 1;
    }

    pub fn discard_env_overlap(&mut self, kit: &ReporterKit, cand: &EnvCandidate) {
        self.delete_hit(&kit, &cand.id);
        self.write_filtered_hit(&cand.gene_id, &cand.header_base, &cand.hdr_revcomp, &cand.hdr_translate, "env-overlap");
        self.discard_env_overlap += 1;
    }

    pub fn write_report(&mut self) {
        self.report_fh.write_all("\n-- Report --\n\n".as_bytes()).expect("Unable to write to report.txt file");
        self.report_fh.write_all(format!("Skipped Env Pseudo Master: {}\n", self.discard_env_pseudo_master).as_bytes()).expect("Unable to write to report.txt file");
        self.report_fh.write_all(format!("Skipped Env Overlap: {}\n", self.discard_env_overlap).as_bytes()).expect("Unable to write to report.txt file");
        self.report_fh.write_all(format!("Skipped Hmm Overlap: {}\n", self.discard_hmm_overlap).as_bytes()).expect("Unable to write to report.txt file");
        self.report_fh.write_all(format!("Skipped No ORF: {}\n", self.discard_non_orf).as_bytes()).expect("Unable to write to report.txt file");
    }
